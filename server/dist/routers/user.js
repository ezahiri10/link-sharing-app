...
export const userRouter = router({
    me: protectedProcedure.query(async ({ ctx }) => {
        const result = await ctx.db.query('SELECT id, email, first_name, last_name, profile_email, image FROM users WHERE id = $1', [ctx.user.id]);
    ...
    updateProfile: protectedProcedure
        ...
        let imageUrl;
        if (avatarBase64) {
            const currentUser = await ctx.db.query('SELECT image FROM users WHERE id = $1', [ctx.user.id]);
