...
export const authRouter = router({
    register: publicProcedure
        ...
        const existingUser = await ctx.db.query('SELECT id FROM users WHERE email = $1', [email]);
        ...
        const user = await ctx.db.query('SELECT id, email, first_name, last_name, image FROM users WHERE id = $1', [userId]);
        return {
            sessionId,
            user: user.rows[0],
        };
    ...
    login: publicProcedure
        ...
        .mutation(async ({ input, ctx }) => {
        const { email, password } = input;
        const result = await ctx.db.query('SELECT id, email, password_hash, first_name, last_name, image FROM users WHERE email = $1', [email]);
